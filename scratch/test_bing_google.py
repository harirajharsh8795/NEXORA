...
try:
    ...
    for i, b in enumerate(bing_results[:3]):
        h2 = b.find("h2")
        p = b.find("p")
        a = h2.find("a") if h2 else None
        print(f"  Result {i+1}: {h2.get_text(strip=True) if h2 else ''}")
        print(f"    URL: {a.get('href', '') if a else ''}")
        print(f"    Snippet: {p.get_text(strip=True) if p else ''}")
except Exception as e:
    print(f"Bing error: {e}")

print("\n--- Testing Google Search HTML ---")
try:
    r_goog = requests.get(f"https://www.google.com/search?q={query}&gbv=1", headers=headers, timeout=8)
    print(f"Google Status: {r_goog.status_code}")
    soup = BeautifulSoup(r_goog.text, "html.parser")
    # In gbv=1 (no-JS Google), search results are in div.g or table
    snippets = soup.find_all("div", class_="BNeawe")
    print(f"Google Text Snippets: {len(snippets)}")
    for i, s in enumerate(snippets[:5]):
        txt = s.get_text(strip=True)
        if len(txt) > 20:
            print(f"  Snippet {i+1}: {txt[:100]}")
except Exception as e:
